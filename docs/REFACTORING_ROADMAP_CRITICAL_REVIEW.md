...
**B15 — Name collision: `dag-ml-core` vs `nirs4all-core` (which is itself two things — the throwaway integration clone AND the future aggregate).**
...
*Correction:* `LOCK-GOV` must explicitly reconcile `dag-ml-core` (contract crate) vs `nirs4all-core` (aggregate vs throwaway clone) — they are NOT the same layer and must not share "core" unqualified, or `L4`/`L5` agents collide. Note: `nirs4all-core` is a **redundant linked git WORKTREE** of `nirs4all` to be retired post-review; the `nirs4all-core` paths in Codex-1 are the same code as `nirs4all`.
...
*Correction:* `CORE-001` "rename lite→core" is renaming a scaffold, not promoting a finished aggregate. `CORE-002` "direct exposition" of 6 upstreams is real implementation work (vendoring/`pub use` vs `libloading`); decide whether datasets is in or out of the default aggregate (today: out); fix the diagrams that place all six (incl. datasets) inside core.
...
- **Cutover context (currently true).** `nirs4all` main `e41362b4` (default legacy, dag-ml selectable, NO publish/tag); `dag-ml` on `feat/native-scoring` (`f58d7bf`) **NOT merged to dag-ml main** — the merge + dag-ml-data lockstep release land AT/after the global refactoring; `nirs4all-core` is a redundant linked git WORKTREE (== `e41362b4`) to be retired post-review; `ADR-17_LEGACY_DROP_HANDOFF.md` documents the 9-step drop. The cutover/legacy-DROP is a first-class lane the roadmap lacks (B7).

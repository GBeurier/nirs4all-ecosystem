...
- `nirs4all-org` was mostly current, but had doc/asset drift:
  - Providers version mismatch between pages.
  - `assets/logos_ecosystem/logo_lite.png` remained unreferenced.
  - License messaging differs between rendered/public policy and repository metadata.
- Follow-up implemented in `nirs4all-org` commit `2dd7885 docs(site): refresh providers card`: providers card now says v0.2.9 and the stale lite logo asset was removed.
...
3. Decide whether live `pls4all` dependencies in `nirs4all-core` are intentional compatibility or should be scheduled for retirement.
4. Render quality/lab components with sample data in the `nirs4all-ui` gallery without modifying the quality-owned component APIs.
5. Resolve the `nirs4all-org` license-message mismatch separately from the static content refresh.

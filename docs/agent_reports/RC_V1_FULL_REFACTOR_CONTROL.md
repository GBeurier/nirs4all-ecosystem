...
| Performance comparison | `n4a-benchmarks perf-compare --repeats 3 --assert-max-ratio python_run=1.25 --assert-max-ratio studio_run=1.35` on Benchmarks `6e4c630`: earlier direct proof showed Python direct `dag-ml/legacy` run ratio `0.730x` and Studio worker `0.707x`. Wave 4AU cutover sweep refreshed the gate: Python direct run ratio `0.748x`, total ratio `0.786x`; Studio worker run ratio `0.680x`, total ratio `0.736x`. Python `scripts/bench_engine_perf.py` from the perf head `5d37b921` is unchanged through current Python `bf242e48`; it refuses unverified dag-ml fallback and smoke-passed `pls_small` with `0.15x` run ratio, `0.96x` RSS ratio, and `0` prediction-count delta. | RC perf gate is required in cutover contracts and is green in Wave 4AU. Evidence has fallback disabled, recorded engines matching requested engines, JSON/Markdown saved under `/tmp/n4a_perf_compare_rc_gate.*`, and a Python direct smoke JSON under `/tmp/n4a_bench_engine_perf_smoke_after_ci_fix_20260703.json`. |
| Core language surfaces | Wave 4K on `RC-v1-nirs4all-core` `29d6d04`: Rust fmt/clippy/tests passed; Python binding tests `54 run, 1 skipped`; WASM tests passed with Linux Node 24 (`13 passed, 2 skipped` full WASM; V1 surface `13 passed, 1 skipped`); V1 Python surface tests `53 passed`; strict core Python parity and Rust parity passed. Wave 4Y moves Core to `2b0d18a`: Methods JS/WASM `make test-js-wasm` passes on EMSDK (`emcc 5.0.7`, Node `22.16.0`), stages `index.js`, `n4m.js`, and `n4m.wasm`; Core `make test-wasm-parity-strict NIRS4ALL_METHODS_ROOT=../RC-v1-methods` passes `15` WASM tests plus TypeScript typecheck. Wave 4AQ moves Core to `ba959a1`: `release-matlab.yml` now blocks package upload on strict MATLAB/Octave parity against the pinned Methods ref, release topology/capability unittest reports `21 tests OK`, workflow YAML parses, and `make -n test-matlab-parity` confirms `NIRS4ALL_LITE_REQUIRE_METHODS_PARITY=1`. Wave 4AU then runs the required aggregate gate with Linux Node, R, Octave, and selected Methods root: Rust tests, Python V1 surface (`53 tests OK`), WASM V1 surface (`14` Node tests plus TypeScript typecheck), R strict parity, and Octave strict parity all pass. | Core language-surface proof is green locally for Rust, Python, WASM, R, and Octave on the selected RC heads. Licensed MATLAB host proof remains distinct from Linux Octave and must be recorded explicitly before GA claims that depend on licensed MATLAB. |
...
- Wave 4S covered normal `rc/**` CI triggers across the remaining selected
  ...
  `dag-ml-data` `616f3e5`, `nirs4all-formats` `32fc87f`,
...
- Cluster GitGuardian remediation cleaned the published active heads and was
  ...
  docs/help. Wave 4AR then adds a CI secret-scan guard. Published branch/tag refs
  now point to clean heads: `main` `aec2a10`, `rc/v1-full-refactor` `9643460`,
  tag `n4a-v1-rc1-2026.07-refactor` `9643460`. Strict scanner-pattern checks over
  the active branch/tag refs are empty; the focused RBAC gate is `24 passed`, full
  cluster pytest is `145 passed`, `1 skipped`, `1 deselected`, and GitHub Actions
  are green for both heads.
...
- Latest pushed/tagged heads after the Wave 4AR runtime-pin and CI refresh batch: Python `bf242e48`,
  dag-ml `a8f6cb3`, dag-ml-data `616f3e5`, Core `ba959a1`,
  Studio `1508242`, Web `5be833d`, UI `69501bd`, Cockpit `6cdc829`,
  Org `2d44265`, Ecosystem uses the current board HEAD, Providers `2cfcca6`,
  Tools `7c5070f`, Cluster `9643460`, Formats `32fc87f`, IO `26963d5`,
...
- Wave 4AO moves Python to `6d6d78bc`, Studio to `a509888`, Web to `5be833d`,
  Org to `2d44265`, and Cockpit to `6cdc829`, with branch and tag
  `n4a-v1-rc1-2026.07-refactor` refreshed for each. Python fixes the macOS CI
  unit failures by making legacy-only unit surfaces explicit
  (`engine="legacy"`) and by accepting both whole-dataset and packed-branch
  snapshots in disjoint feature merge. Local gates passed targeted regression
  reproduction (`21 passed`), expanded touched-unit coverage (`261 passed`),
  full `tests/unit/` (`6918 passed`, `116 skipped`; non-parity unit suite),
  `ruff`, and full `mypy nirs4all`. Studio release archives and Docker builds
  now pin `nirs4all`, `dag-ml`, and `dag-ml-data` to immutable selected RC SHAs
  instead of moving branch tarballs, and the release workflow fails closed if a
  runtime dependency ref is not a 40-character SHA or if a source URL uses
  `/refs/heads/`. Web/Org close stale public `n4a-web`/domain wording, and
  Cockpit now accounts for MATLAB/Octave as part of the tracked aggregate
  release surface without inventing a false asset-level target.
- Wave 4AK/4AL/4AM/4AN record parallel agent review: UI/Web client-only and
  shared `nirs4all-ui` consumption are confirmed; all five V1 language surfaces
...
- Wave 4AR moves Python to `bf242e48` after the Windows CI parquet atomic-replace
  failure on `5e7a0e4b`. The fix retries transient `os.replace` `PermissionError`
...
- Wave 4AU reran the required non-full cutover sweep after the Wave 4AT full
  parity proof. With Linux nvm Node/npm, conda R, conda Octave, and selected
  `RC-v1-methods`, `python3 scripts/n4a_cutover_gates.py run --workspace-root
  /home/delete/nirs4all --skip pyref_oracle_full --json` reports
  `passed: true` and `failed_required: []`. The `lite_v1_surfaces` gate now
  includes strict R parity and strict Octave parity instead of accepting missing
...
Remaining non-Python proof debt is broader release-environment and hosting
coverage, not accepted Python parity debt:

- R strict runtime parity and Octave strict runtime parity are now recorded in
  Wave 4AU for the selected RC core/methods heads. Missing R/Octave host
  execution remains a blocker for any later selected head movement.
- Licensed MATLAB runtime proof remains outside the local Linux Octave proof;

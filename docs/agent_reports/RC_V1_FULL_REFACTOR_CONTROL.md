...
| --- | --- | --- |
| Python parity harness | Python runtime proof head `3d568ab` has full `pyref_oracle_full` green on the selected RC stack (`dag-ml` `7f86a9b`, `dag-ml-data` `e681685`): `659 passed, 227 deselected, 1530 warnings` in `2037.46s`; no parity tests skipped or xfailed. The current pushed Python head `884a196` adds RC-aware `dag-ml` CLI discovery and targeted parity/native gates passed (`49`, `30`, `6`, `8`, and `6` case batches). Marker audit still reports classified skip call sites by AST, not realized parity skips. | Full numerical parity is clean on the earlier runtime code head; the current targeted batch is green with `fallback=0`. Rerun full parity after this integrated batch before final production cutover. |
...
- Python `nirs4all` RC head previously reached `a103fd2 docs(parity): refresh rc compatibility ledger`; Wave 4U later moves it to `884a196` with RC-aware `dag-ml` CLI resolution. The current full parity proof remains the runtime code head `3d568ab`: on the selected RC stack (`dag-ml` `7f86a9b`, `dag-ml-data` `e681685`), `pyref_oracle_full` passed with `659 passed, 227 deselected, 1530 warnings` in `2037.46s`; no parity tests were skipped or xfailed. The preceding `42448821` result with `14 skipped / 6 xfailed` is superseded.
...
- Wave 4U moves the selected RC batch to Python `884a196`, datasets `59b34f5`,
  ...
  Full Python-reference parity was intentionally not rerun during the small
  fixes; it is the next large-batch gate.
...
- Skip/xfail audit is recorded in `RC_SKIP_XFAIL_AUDIT.md`: Studio operator skips and Python registry skips have been burned down; Python full parity on runtime head `3d568ab` reports no skipped or xfailed parity tests. Current Python head `884a196` is targeted-gate green but has not rerun full parity. Remaining skip risk is outside this gate: Studio optional/environment categories and methods/language binding environments.

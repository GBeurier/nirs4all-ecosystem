...
| --- | --- | --- |
| Python parity harness | After `42448821`, live PipelineCase registry skip debt is 0 and strict xfails are 6. Five former strict xfails now pass targeted dual-engine gates: augmentation x3, feature-augmentation replace views, and Optuna finetune. Full `tests/integration/parity`: `853 passed, 14 skipped, 6 xfailed` in `2281.65s`. Marker audit still reports sanctioned `registry_skip` call sites by AST. | Full parity is green but not clean. Remaining skips/xfails still require release treatment before production flip. |
...
| Web | After `69c0b75`, deploy Pages and RC CI gates both require `npm ci`, client-side-only contract `2 passed`, typecheck, Vitest `134 passed`, `validate:catalog`, strict `check:lite-shim`, `build:single`, `build`, and browser smoke `rt-fallback`. | Web RC is a static/browser-only app with no backend runtime and no intentional third-party runtime requests; vendored `nirs4all-core` shim drift is now a blocking gate. |
...
| Core language surfaces | Wave 4K on `RC-v1-nirs4all-core` `29d6d04`: Rust fmt/clippy/tests passed; Python binding tests `54 run, 1 skipped`; WASM tests passed with Linux Node 24 (`13 passed, 2 skipped` full WASM; V1 surface `13 passed, 1 skipped`); V1 Python surface tests `53 passed`; strict core Python parity and Rust parity passed against `RC-v1-methods` dev-release `libn4m.so.2.0.0`. | R/Rscript and Octave are unavailable locally, so R and MATLAB/Octave execution remain environment gates. WASM methods execution skips remain until the local methods JS/WASM package is built or CI runs that environment. |
...
- Web RC head now includes `69c0b75 ci(web): harden pages deployment gate`; the public `web.nirs4all.org` build remains GitHub Pages/static/WASM-only and both RC CI and Pages deploy now run client-only, typecheck, Vitest, catalog, strict lite/core shim, build, single-file build, and one browser smoke gate.
...
- Core RC head `29d6d04` now has fresh Wave 4K language-surface evidence: Rust/Python/WASM topology gates pass, and strict Python/Rust core parity passes when pointed at the selected `nirs4all-methods` RC build. R and Octave execution remain unavailable in this local environment.
- `nirs4all-providers` RC head now includes `5146908 ci(providers): add neutral contract gate`; the gate runs Ruff, mypy, hermetic tests, conformance tests, and neutral contract validation without forcing optional provider backings into base install.
...
- `nirs4all-cockpit` RC head now includes `33c91c3 docs(topology): track dag-ml binding rc surfaces`; planned RC surfaces include `nirs4all-ui` `8f9f2f6`, `nirs4all-providers` `5146908`, `nirs4all-tools` `7c5070f`, plus planned Python/WASM/R binding targets for `dag-ml` and `dag-ml-data`.
...
- Latest pushed/tagged heads after the publication/security/parity/CI refresh batch: Python `3d568ab`, dag-ml `7f86a9b`, dag-ml-data `e681685`, Core `29d6d04`, Studio `8141e2e`, Web `69c0b75`, UI `8f9f2f6`, Cockpit `33c91c3`, Org `9417073`, Ecosystem uses the current board HEAD, Providers `5146908`, Tools `7c5070f`, Cluster `e843073`, Formats `86218e6`, IO `c064ecf`, Datasets `d9cbd99`, Methods `44cc948`, Repository `534c907`, Benchmarks `45f4cf7`, Papers `acde191`.

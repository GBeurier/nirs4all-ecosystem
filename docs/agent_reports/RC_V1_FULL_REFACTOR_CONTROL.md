...
- Python `nirs4all` RC head now includes `42448821 fix(parity): handle disabled chart steps in dagml`; disabled chart-only pipeline steps are stripped on dag-ml only when chart rendering is off, requested charts fail loud, public preprocessing examples now run on dag-ml, and full parity passes with `853 passed, 14 skipped, 6 xfailed`.
...
- Web RC head now includes `ae507ab fix(web): remove third-party runtime requests`; the public `web.nirs4all.org` build remains GitHub Pages/static/WASM-only and the client-only contract now rejects remote runtime scripts and remote CSS imports.
...
- Cluster GitGuardian remediation was upgraded from tip cleanup to a targeted history rewrite and refreshed after the latest alert. Published refs now point to rewritten clean history: `main` `727480c`, `rc/v1-full-refactor` `c4df557`, tag `n4a-v1-rc1-2026.07-refactor` `e0784fa` peeled to `c4df557`. Scanner-pattern history scan over the rewritten refs is empty; cluster RC gate is `142 passed, 1 skipped, 1 deselected`.
...
- Latest pushed/tagged heads after the security/parity/perf refresh batch: Python `42448821`, Studio `1d1ded5`, Web `ae507ab`, Benchmarks `45f4cf7`, Cluster `fef2f5c`; Ecosystem uses the current board HEAD for this note.
...
- Skip/xfail audit is recorded in `RC_SKIP_XFAIL_AUDIT.md`: Studio operator skips and Python registry skips have been burned down; Python full parity is refreshed on `42448821` with `14 skipped / 6 xfailed`, which still blocks production-flip proof until each item is implemented or moved to an explicit optional-environment/legacy-bug gate.
...
| RC-I | Codex/GPT-5.5 `019f2286-ba97-7512-8a9d-88b0995a85dc` | Cluster client/server scheduler, DAG rights, minimal client contracts. | integrated in rewritten cluster RC head `c4df557`; GitGuardian history rewrite complete for published refs | `RC_I_CLUSTER_SCHEDULER.md`, `RC_SECURITY_GITGUARDIAN_CLUSTER.md` |

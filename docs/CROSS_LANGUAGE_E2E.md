...
| `e2e-converter-legacy-save-predictions-web` | The public Python migration CLI proves that unavailable legacy semantic lowering refuses before creating output; it does not substitute a rerun or Web rendering for conversion evidence. |
...
| `e2e-formats-io-datasets-methods-language-bindings` | Formats/IO assemble reference datasets, methods bindings compare Python/R/WASM/native prediction evidence, the nirs4all-core MATLAB/Octave release archive gate is verified, and the official Rust pre-release binding is explicitly outside the prediction-parity matrix. |

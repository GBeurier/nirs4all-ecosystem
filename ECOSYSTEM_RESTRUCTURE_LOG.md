...
- 2026-06-12 local follow-up: `nirs4all-methods` CI follow-up `e0e8000` declares the existing `n4m_config_set_rng_kind` / `n4m_config_get_rng_kind` C ABI functions in the public PLS header and removes signed index conversions from the threading test. Local C/C++ build and `n4m_tests` pass with FITPACK disabled; exact GCC12/Clang16 sanitizer validation is delegated to GitHub Actions because those compilers are not installed locally.
...
| done | Fix methods CI regressions after WASM follow-up | Added missing public RNG config declarations and removed signed vector indexing from the threading test; local C/C++ build and `n4m_tests` pass, GitHub Actions are running on commit `e0e8000`. |
...
- Extend the new execution parity gate beyond npm/WASM: run the same portable `nirs4all` JSON/YAML fixtures through native `nirs4all-lite` and every host binding once their executable pipeline runners exist.

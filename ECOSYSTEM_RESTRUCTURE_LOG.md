...
- 2026-06-12 local follow-up: `nirs4all-datasets` WASM was rechecked from source. `wasm-pack build` passes for `--target nodejs` and `--target web`, and the Node smoke confirms `resolve`, `sha256`, and `abiVersion`. The README now uses the real package name `@nirs4all/nirs4all-datasets-wasm`.
...
- 2026-06-12 final follow-up: GitHub Actions are green on `nirs4all-datasets` commit `45020ad`, `nirs4all-lite` commit `286ad08`, `nirs4all-web` commit `16b500a`, and `nirs4all-methods` commit `0a1b9a2`. `nirs4all.org` and `https://gbeurier.github.io/` both respond with HTTP 200 from GitHub Pages.
...
- 2026-06-12 continuation finalization: `nirs4all-lite` commit `2b9d8a5` is green on CI run `27392792796`, including the new source-built methods WASM + native `libn4m` `strict-parity` job. `nirs4all-web` commit `4c771c2` is green on Pages run `27392548033`. `nirs4all-ecosystem` gitlinks were advanced to `nirs4all-datasets` `45020ad`, `nirs4all-methods` `0a1b9a2`, `nirs4all-lite` `2b9d8a5`, and `nirs4all-web` `4c771c2`.
...
| done | Fix and revalidate datasets CI after WASM follow-up | Latest `nirs4all-datasets` workflows are green for CI, ABI Surface, Site, and version-sync on commit `45020ad`. |
| done | Fix methods CI regressions after WASM follow-up | Latest `nirs4all-methods` commit `0a1b9a2` is green for CI, CUDA build, Parity gate, Cross-binding parity, ABI Surface, Sanitizers, Coverage, docs, and version-sync. |
...
- Extend the new execution parity gate beyond npm/WASM/Python: run the same portable `nirs4all` JSON/YAML fixtures through native Rust, R, and MATLAB/Octave aggregate bindings once their upstream method surfaces can execute the portable KS/SNV/Savitzky-Golay/PLS subset without reimplementing kernels.
